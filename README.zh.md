...
本仓库发布两套评测集：**460 题全集**（`data/`，由 `scripts/reproduce.py` 复算）和从中按已公开规则抽取的 **200 题分层子集**（`subset-200/`，由 `scripts/make_subset.py` 抽取、`scripts/reproduce_200.py` 复算）。报告 200 道题的论文使用的是子集，见下文「200 题评测子集」一节。
...
`subset-200/` 是从 `data/` 的 420 道可答题中按固定规则抽出的 200 道评测集：按题型与标准来源文档所属地市分层，名额按比例分配，固定随机种子 0。选取只读这两个字段——不读任何分数、答案、引用或裁判理由——因此保留哪些题不可能取决于任一系统在这些题上的表现。规则即 `scripts/make_subset.py`，可逐字节重建该目录：

```bash
python3 scripts/make_subset.py --check     # 校验 subset-200/ 与规则产物一致
python3 scripts/reproduce_200.py           # 复算其所报数字
```

| 题型 | n | DeepKnown | Gemini | 分差 | 95% CI |
|---|---|---|---|---|---|
| 总体 | 200 | 97.7 | 88.1 | +9.6 | [5.7, 13.8] |
| 简单 | 106 | 99.0 | 91.1 | +7.9 | [3.2, 13.3] |
| 复杂 | 81 | 96.8 | 86.6 | +10.2 | [3.5, 18.0] |
| 部分 | 13 | 92.3 | 73.1 | +19.2 | [7.7, 30.8] |

种子在抽取前定下。在种子 0–9 上，200 题抽样的总体分差范围为 +7.6 到 +12.7、中位数 +11.3，而全部 420 道可答题上为 +11.0：本子集的 +9.6 应理解为该分布中的一次抽样，而非一项独立结论；要取最紧的估计，请用全集。
